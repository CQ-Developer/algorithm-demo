...
class DFS {
    ...
    public static int buy(int n, int x, int[] price, int[] discount, int[] happy) {
        int m = 0, k = 0;
        for (int i = 0; i < n; i++) {
            price[i] -= discount[i];
            m += price[i];
    ...
    private static int dfs(int n, int x, int[] price, int[] discount, int[] happy, int[][][]f, int i, int j, int k, int sum) {
        if (i == n) {
            return j >= k - x ? sum : 0;
        }
        if (f[i][j][k] != 0) {
